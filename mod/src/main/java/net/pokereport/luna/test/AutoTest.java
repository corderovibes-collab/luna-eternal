...
public final class AutoTest {
        ...
        try (Connection c = db.connection()) {
            ...
            try {
                ...
                for (String sql : List.of(
                    ...
                    "DELETE pr FROM pase_reclamo pr JOIN player p "
                        + "ON p.player_id = pr.player_id WHERE p.mc_uuid = ?",
                    "DELETE pj FROM pase_jugador pj JOIN player p "
                        + "ON p.player_id = pj.player_id WHERE p.mc_uuid = ?",
    ...
    private void testPase(long jugador) throws Exception {
        ...
        for (int n = 0; n < net.pokereport.luna.pase.PaseNivel.MAX; n++) {
            suma += net.pokereport.luna.pase.PaseNivel.coste(n);
        }
        check("la curva del pase suma exactamente el total",
              suma == net.pokereport.luna.pase.PaseNivel.total());
        check("con la XP total se llega al nivel maximo",
              net.pokereport.luna.pase.PaseNivel.nivelDe(
        ...
        for (int n = 0; n <= net.pokereport.luna.pase.PaseNivel.MAX; n++) {
            ...
            if (n < net.pokereport.luna.pase.PaseNivel.MAX
                    && net.pokereport.luna.pase.PaseNivel.nivelDe(
                            net.pokereport.luna.pase.PaseNivel.acumulada(n + 1) - 1)
                        != n) {
                inversa = false;
            }
        }
        ...
        check("el descanso acumulado no pasa de tres dias",
              net.pokereport.luna.pase.PaseNivel.TOPE_MAXIMO
                  == net.pokereport.luna.pase.PaseNivel.TOPE_DIARIO
                     * net.pokereport.luna.pase.PaseNivel.DIAS_DESCANSO);

        // ---- la XP de la Torre -----------------------------------------
        boolean sube = true;
        long anterior = -1;
        for (int r = 1; r <= 200; r++) {
            long x = net.pokereport.luna.pase.PaseXp.torre(r);
            if (x < anterior) {
                sube = false;
            }
            anterior = x;
        ...
        var temporada = svc.temporada();
        check("hay una temporada de pase en curso", temporada.numero() >= 1);

        // ⚠⚠⚠ EL TOPE DIARIO SE APLICA DE VERDAD. Es la comprobacion que
        //    convierte el diseño en una garantia: se piden diez topes de golpe
        //    y solo puede entrar uno.
        var antes = svc.estado(jugador);
        long pedido = net.pokereport.luna.pase.PaseNivel.TOPE_MAXIMO * 10L;
        var g = svc.ganar(jugador, pedido);
        check("el tope diario recorta una concesion desmedida",
              g.concedida() <= net.pokereport.luna.pase.PaseNivel.TOPE_MAXIMO
                  && g.concedida() < pedido);
        var g2 = svc.ganar(jugador, pedido);
        check("una vez topado, el dia no da ni una XP mas", g2.concedida() == 0);

        var despues = svc.estado(jugador);
        check("la XP del pase sube exactamente lo concedido",
              despues.xp() == antes.xp() + g.concedida());
        check("el nivel del pase sale de la XP y no de una columna",
              despues.nivel()
